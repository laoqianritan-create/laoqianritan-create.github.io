// site/js/chronicle-year.js · 编年史单年页通用脚本
// 在 79 个年页之间共享:走势图渲染 + 时代背景 fetch + 主题切换
// 年份从 DOM 自动检测(chartYear<Y> 容器 + section[data-era])
// 数据路径在开发版(/site/chronicle/)与部署版(/us-market/chronicle/)之间自动切换
(function () {
  ...
  function getYearChartOption(data) {
    ...
    return {
      ...
      yAxis: {
        type: 'value', scale: true, name: '标普500',
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          let html = `${dateStr}<br/>标普500: <b>${p.value[1].toFixed(2)}</b>`;
          const ev = eventByDate.get(dateStr);
          if (ev) html += `<br/><span style="color:${sec};">事件: ${ev.label}</span>`;
          return html;
        },
      },
      series: [{
        name: '标普500日线', type: 'line', smooth: false, showSymbol: false, sampling: 'lttb',
  ...
  function renderEra(era) {
    ...
    if (titleEl && era.title) titleEl.textContent = '时代背景 · ' + era.title;
  ...
  (function loadEra() {
    ...
    fetch(`${DATA_BASE}/era_data/${eraId}.json?v=${Date.now()}`)
      ...
      .catch(err => {
        ...
        if (el) el.innerHTML = '<p style="color:var(--text-secondary);">时代背景加载失败</p>';
      });
  })();
})();
