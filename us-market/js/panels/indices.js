...
export function initPanelVixeq(vixData, vixeqData) {
  ...
  function getOption() {
    const gridColor = cssVar('--chart-grid') || '#f0f0f0';
    const grayColor = cssVar('--gray') || '#999';
    const vixColor = '#1a1a1a';     // VIX 主线沿用看板主线黑
    const vixeqColor = '#2563eb';   // VIXEQ 蓝（项目品牌蓝）

    return {
      animation: false,
      grid: { left: 56, right: 30, top: 36, bottom: 60 },
      legend: getLineLegendConfig({
        data: ['VIX（市值加权）', 'VIXEQ（等权）'],
      }),
      xAxis: {
        type: 'time',
        min: vixeqStart,
      ...
      tooltip: {
        ...
        formatter: params => {
          ...
          if (vix?.value && vixeq?.value && vix.value[1] != null && vixeq.value[1] != null) {
            ...
            lines.push(`等权 − 市值: <b style="color:${diffColor}">${diff >= 0 ? '+' : ''}${formatNumber(diff, 2)}</b>`);
          }
  ...
  renderMetricStrip('vixeqSummary', [
    buildMetricCard('VIX 最新', latestVix ? formatNumber(latestVix[1], 2) : '--', latestVix?.[0] || ''),
    buildMetricCard('VIXEQ 最新', latestVixeq ? formatNumber(latestVixeq[1], 2) : '--', latestVixeq?.[0] || ''),
    buildMetricCard(
      '等权 − 市值',
      spread != null ? `${spread >= 0 ? '+' : ''}${formatNumber(spread, 2)}` : '--',
      spread != null && spread > 5 ? '等权波动率显著高于市值（个股层面恐慌）' : '价差正常',
    ),
    buildMetricCard('VIXEQ 起点', vixeqStart || '--', 'CBOE 2014.6 起'),
  ]);
}
